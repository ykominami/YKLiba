class Util {
  ...
  static debug_array(name, array){
    ...
    if( Util.get_log_level() <= Util.DEBUG() ){
      Util.display_debug(`${name}`)
      array.map( item => Util.display_debug(item));
    }
  }
  static debug(mes){
    if( Util.get_log_level() <= Util.DEBUG() ){
      Util.display_debug(mes)
    }
  }
  static debug_x(name, value){
    if( Util.get_log_level() <= Util.DEBUG() ){
      Util.display_debug(`${name}`)
      Util.display_debug( value)
    }
  }
  static debug_1(name, value){
    if( Util.get_log_level() <= Util.DEBUG() ){
      Util.display_debug(`${name}=${value}`)
    }
  }
  static debug_2(name, value, name2, value2){
    if( Util.get_log_level() <= Util.DEBUG() ){
      Util.display_debug(`${name}=${value} ${name2}=${value2}`)
    }
  }
  ...
  static warn_1(name, value){
    if( Util.get_log_level() <= Util.WARN() ){
      display_log(`${name}=${value}`)
    }
  }
  static warn_2(name, value, name2, value2){
    if( Util.get_log_level() <= Util.WARN() ){
      display_log(`${name}=${value} ${name2}=${value2}`)
    }
  }
  ...
  static error_array(name, array){
    if( Util.get_log_level() <= Util.ERROR() ){
      display_log(`${name}`)
      array.map( item => display_log(item));
    }
  }
  static error(mes){
    if( Util.get_log_level() <= Util.ERROR() ){
